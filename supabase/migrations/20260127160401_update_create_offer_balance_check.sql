...
BEGIN
    -- 0. VALIDACIÓN DE SALDO (Billetera Lizi)
    v_commission := public.calculate_commission(p_total_price);
    
    SELECT balance, frozen_balance INTO v_balance, v_frozen
    FROM public.seller_wallets
    WHERE seller_id = v_seller_id;

    IF v_balance IS NULL OR (v_balance - v_frozen) < v_commission THEN
    ...
    v_data := jsonb_build_object('listId', p_shopping_list_id, 'offerId', v_offer_id, 'type', 'new_offer');
