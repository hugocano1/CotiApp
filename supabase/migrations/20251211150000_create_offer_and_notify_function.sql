CREATE OR REPLACE FUNCTION public.create_offer_and_notify(
    ...
    p_items jsonb -- Se espera un array de objetos JSON: [{item_name, quantity, unit, brand, unit_price, list_item_id}, ...]
)
RETURNS uuid -- Devuelve el ID de la nueva oferta
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
DECLARE
...
BEGIN
    ...
    -- 3. Obtener datos necesarios para la notificación (del comprador y de la lista)
    ...
    -- Si no se encuentra el comprador o la lista, no se puede notificar, pero la oferta ya se creó.
    IF v_buyer_id IS NULL THEN
        -- Opcional: registrar este caso en una tabla de logs
        RETURN v_offer_id;
    END IF;

    -- 4. Obtener el nombre de la tienda del vendedor para el mensaje
    SELECT s.name INTO v_seller_name FROM public.stores s WHERE s.seller_id = v_seller_id LIMIT 1;
    
    -- 5. Preparar el contenido de la notificación
    v_title := '¡Has recibido una nueva oferta! 报价'; -- Chino para "cotización"
    v_body := 'La tienda "' || COALESCE(v_seller_name, 'un vendedor') || '" te ha enviado una oferta para tu lista "' || v_list_title || '".';
    v_data := jsonb_build_object('listId', p_shopping_list_id, 'offerId', v_offer_id);
